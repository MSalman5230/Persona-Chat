...
async function promoteFallbackDefault(
	...
	const fallbackQuery = database
		...
		.where(
			excludeId
				? and(eq(agents.userId, userId), ne(agents.id, excludeId))
				: eq(agents.userId, userId)
		)
		.orderBy(asc(agents.createdAt))
		.limit(1);
	const [fallback] = await fallbackQuery;
	const fallbackId = fallback?.id ?? (excludeId ? (await getAgentRow(userId, excludeId, database))?.id : undefined);
...
export async function updateAgent(
	...
	return db.transaction(async (tx) => {
		...
		const nextDefault = current.isDefault && parsed.isDefault === false ? true : parsed.isDefault;
