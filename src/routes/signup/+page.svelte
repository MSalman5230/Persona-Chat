...
<main class="flex min-h-dvh items-center justify-center bg-background px-4 py-8 text-text-primary">
	<section class="w-full max-w-[28rem] rounded-lg border border-border-subtle bg-surface-container-low p-5">
		<div class="mb-5">
			<h1 class="font-headline-md text-headline-md text-primary">Sign up</h1>
			<p class="mt-1 font-body-sm text-body-sm text-text-muted">Create your Persona account.</p>
		</div>
		...
		<p class="mt-5 text-center font-body-sm text-body-sm text-text-muted">
			...
			<a class="text-primary underline-offset-4 hover:underline" href={resolve(`/login?redirectTo=${encodeURIComponent(data.redirectTo)}`)}>
				Sign in
			</a>
		</p>
	</section>
